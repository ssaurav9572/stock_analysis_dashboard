...
def show_charts(stock, company):
    ...
    if not hist.empty:
        ...
        if chart_type == "Line Chart":
            ...
            if show_sma50: fig.add_trace(go.Scatter(x=hist.index, y=hist["SMA_50"], name="50-day SMA"))
            if show_sma200: fig.add_trace(go.Scatter(x=hist.index, y=hist["SMA_200"], name="200-day SMA"))
            if show_ema50: fig.add_trace(go.Scatter(x=hist.index, y=hist["EMA_50"], name="50-day EMA"))
            if show_ema200: fig.add_trace(go.Scatter(x=hist.index, y=hist["EMA_200"], name="200-day EMA"))
            fig.update_layout(title=f"{company} - Line Chart", yaxis_title="Price (₹)")
            ...
            if show_ema50: fig.add_trace(go.Scatter(x=hist.index, y=hist["EMA_50"], name="50-day EMA"))
            if show_ema200: fig.add_trace(go.Scatter(x=hist.index, y=hist["EMA_200"], name="200-day EMA"))
            fig.update_layout(xaxis_rangeslider_visible=False, title=f"{company} - Candlestick Chart", yaxis_title="Price (₹)")
            st.plotly_chart(fig, use_container_width=True)
    else:
